...
def set_determinism(seed: int) -> None:
    ...
    try:
        torch.use_deterministic_algorithms(False)  # deterministic scatter_add on the TIED embedding grad is ~40% slower; SDPA is non-deterministic anyway so bit-determinism is unattainable
    except Exception:
        pass
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # The model self-compiles its forward via a BOUND-METHOD torch.compile inside
    # RalphBase.forward (see model/ralph_base.py): it compiles self._forward_impl,
    # which returns a plain function (NOT an nn.Module), so it stays out of _modules
    # and the saved state_dict is unchanged (no "_orig_mod." prefix -> op4 strict-load
    # safe). The in-model compile is gated on CUDA + RALPH_NO_COMPILE != "1". We do NOT
    # wrap the model here -- that would double-compile the forward.
    fwd = model
    ...
    for step in range(cfg.total_steps):
        ...
        step_loss_acc = torch.zeros((), device=device)
        for accum in range(cfg.grad_accum_steps):
            ...
            step_loss_acc = step_loss_acc + loss.detach()
            tokens_seen += cfg.micro_batch_size * cfg.seq_len

        step_loss = (step_loss_acc / cfg.grad_accum_steps).item()
        ...
        if False:  # intermediate checkpoints removed (op1 recipe-match)
